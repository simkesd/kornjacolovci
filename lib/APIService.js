...
var APIService = (function() {
    ...
    function requests(req, res) {
        var id = req.params.id;
        if(id) {
            query('SELECT * from requests where id = ?', [id], res);
        }else {
            query('SELECT * from requests', [], res);
        }
    }

    function requestsEdit(req, res) {
        var id = req.params.id;
        query('UPDATE requests SET quantity = ? WHERE id = ? ', [req.body.quantity, id]);
    }
    ...
    function requestsAdd(req, res) {
        query('INSERT INTO requests (quantity) VALUES (?)', [req.body.quantity]);
    }
