...
var APIService = (function() {
    ...
    function requests(req, res) {
        var id = req.params.id;
        if(id) {
            query('SELECT request.*, animal.name as animal_name from request LEFT JOIN animal as a on a.ID = request.animal_ID where ID = ?', [id], res);
        }else {
            query('SELECT request.*, animal.name as animal_name from request LEFT JOIN animal on animal.ID = request.animal_ID', [], res);
        }
    }
