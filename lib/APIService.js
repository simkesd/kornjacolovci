...
var APIService = (function() {
    ...
    function userOffers(req, res) {
        var id = req.params.id;
        query('SELECT * FROM offer WHERE offerer_ID = ?', [id], res);
    }

    function userRequests(req, res) {
        var id = req.params.id;
        //query('SELECT * FROM request WHERE user_ID = ?', [id], res);
        query('SELECT r.*, a.name as animal_name from request as r LEFT JOIN animal as a on a.ID = r.animal_ID where r.user_ID = ?', [id], res);
    }

    function userActiveRequests(req, res) {
        var id = req.params.id;
        query('SELECT r.*, a.name as animal_name from request as r LEFT JOIN animal as a on a.ID = r.animal_ID WHERE delivery_date > now() AND offer_ID is NULL AND user_ID = ?', [id], res);
    }

    function userLogin(req, res) {
        query('SELECT * FROM user WHERE username = ? AND password = ?', [req.body.username, req.body.password], res);
    }
    ...
    function requestsEdit(req, res) {
        var id = req.params.id;
        query('UPDATE request SET offer_ID = ? WHERE ID = ? ', [req.body.offer_ID, id], res);
    }
    ...
    function requestsActive(req, res) {
        query('SELECT r.*, a.name as animal_name from request as r LEFT JOIN animal as a on a.ID = r.animal_ID WHERE delivery_date > now() AND offer_ID is NULL', [], res);
    }

    function requestOffers(req, res) {
        var id = req.params.id;
        query('SELECT * FROM offer WHERE request_ID = ?', [id], res);
    }
    ...
    function sell_offers(req, res) {
        var id = req.params.id;
        if(id) {
            query('SELECT sell_offer.*, u.user_name, a.name as animal_name from sell_offer LEFT JOIN user as u on u.ID = user_id LEFT JOIN animal as a on a.ID = sell_offer.animal_ID where sell_offer.ID = ?', [id], res);
        }else {
            query('SELECT sell_offer.*, u.user_name, a.name as animal_name from sell_offer LEFT JOIN user as u on u.ID = user_id LEFT JOIN animal as a on a.ID = sell_offer.animal_ID', [], res);
        }
    }

    function sell_offersEdit(req, res) {
        var id = req.params.id;
        query('UPDATE self_offer SET quantity = ? WHERE ID = ? ', [req.body.quantity, id], res);
    }

    function sell_offersDelete(req, res) {
        var id = req.params.id;
        query('DELETE FROM sell_offer WHERE ID = ? ', [id], res);
    }

    function sell_offersAdd(req, res) {
        query('INSERT INTO sell_offer (animal_id, weight, quantity, place, transport, user_id, description) VALUES (?, ?, ?, ?, ?, ?, ?)', [req.body.animal_id, req.body.weight, req.body.quantity, req.body.place, req.body.transport, req.body.user_id, req.body.description], res);
    }
    ...
    constructor.prototype.userOffers = userOffers;
    constructor.prototype.userRequests = userRequests;
    constructor.prototype.userLogin = userLogin;
    constructor.prototype.userActiveRequests = userActiveRequests;
    ...
    constructor.prototype.sell_offers = sell_offers;
    constructor.prototype.sell_offersEdit = sell_offersEdit;
    constructor.prototype.sell_offersDelete = sell_offersDelete;
    constructor.prototype.sell_offersAdd = sell_offersAdd;
